...
class WeatherProcessor:
    def __init__(self):
        ...
        #adjust login info if command-line arguments have been passed.
        try:                                
            opts, args = getopt.getopt(sys.argv[1:], "f:", ["ftp="])
        except getopt.GetoptError:
            print("Invalid arguments given. Exiting.")            
            sys.exit(2)
        for opt, arg in opts:
            if opt in ("-f", "--ftp"):
                args = arg.split("//")
                ...
                if args != None:
                    self.ftpAddy = arg[0]
                    self.ftpLogin = arg[1]
                    self.ftpPassword = arg[2]
        
        #loop in each config array -- as profiles
        #self.path = "/var/weather/"
        self.path = "/usr/local/lib/weather/"
        tmp = open(self.path + "do_not_touch", "w")
        tmp.write("This file helps weather_processor.py monitor its run condition. Please do not touch it.\n")
        tmp.close()
...
#END OF __init__. Functions and classes begin here. ###
    ...
    def stillResize(self, i, newSize, path, name, callLetters, destination):
        ...
        if i == 1 and image.size != tSize:          #don"t resize if it"s already the right size.
            newName = name
            try:
                image = image.resize(tSize, Image.ANTIALIAS)
            except:
    ...
    def animate(self, name, images, size, delay = "100", endDelay = "0"):
        ...
        if tmp != 0:
            ...
            self.loopOptimize(name)
            return True

    #######################################
    def loopOptimize(self, path):
        thisIn = open(path, "r")
        thisOut = open("tmp", "w")
        processObject = subprocess.Popen(["gifsicle", "-O2", "-w"], stdin=thisIn, stdout=thisOut)
        thisIn.close()
        thisOut.close()
        try:
            move("tmp", path)
        except:
            self.log(path + " failed to optimize.")
    ...
    class img:
        ...
        def log (self, error):
            logPath = "/usr/local/lib/weather/error.log"
            logFile = open(logPath, "a")
            logFile.write(strftime("%Y-%m-%d %H:%M:%S ", gmtime()) + error + "\n")
            logFile.close()
